 ...
 *   3-1. slotLocks / phoneLocks 컬렉션 삭제 (동시 예약 직렬화용 잠금 문서 — 내용 없는 표식이라 지워도 안전)
...
const locks = [
  ...(await listAll(token, 'slotLocks')),
  ...(await listAll(token, 'phoneLocks')),
];
...
console.log(`잠금 문서(slotLocks·phoneLocks): ${locks.length}건`);
...
for (const doc of locks) {
  await deleteDoc(token, doc.name);
}
console.log(`  잠금 문서 ${locks.length}건 삭제`);
