...
export function StaffAddParticipantForm({
  ...
  // 추가할 사람이 없을 때 방해되지 않게 기본은 접어 둔다
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setMessage(
      `${result.reservation.participantName} 님을 추가했습니다. (도착 확인 상태)`,
    );
    setName('');
    setPhone('');
    setGradeOrAge('');
    ...
    >
      <div className="detail-row">
        ...
        <button
          ...
          aria-expanded={open}
          aria-label={open ? '현장 추가 입력칸 접기' : '현장 추가 입력칸 열기'}
      ...
      {open ? (
        <>
          <p className="hint-text">
            {slotLabel}에 바로 추가합니다. 시간이 지난 회차도, 정원을 넘겨도
            추가할 수 있습니다. 이름만 필수입니다.
          </p>
